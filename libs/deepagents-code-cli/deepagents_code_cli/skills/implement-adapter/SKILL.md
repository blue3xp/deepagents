...
1.  **Analyze and Identify Interfaces for Refactoring**:
    ...
    *   **Check for Private Libraries**: Read the `Use Private Libraries` skill. If the legacy code in Reference Codebase uses standard libraries (e.g., `requests`, `urllib`) that have a private equivalent defined in that skill (e.g., `private_net_lib`), note this. You MUST use the private library when rewriting the code in the Adapter.
...
3.  **Refactor & Implement Code**:
    ...
    *   **Refactor Logic**:
        ...
        *   **Apply Private Libraries**: If the reference code uses external libraries, check the `Use Private Libraries` skill to see if a private library should be used instead.
